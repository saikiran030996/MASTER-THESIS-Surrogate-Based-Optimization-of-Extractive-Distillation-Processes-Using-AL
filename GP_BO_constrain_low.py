...
final_df.to_csv('data_constrain_low.csv')
...
dataset = pd.read_csv("data_constrain_low.csv")
input_name = ["NStage", "RR", "TopPres", "StoF"]
X = dataset[input_name].values
y_puri, y_duty = dataset["DIST_C4H8_T1"].values, dataset["RebDuty_T1"].values
X_initial = X
y_initial = y_duty
...
while iter_index<=50:
    ...
    y_prediction_purity = model1.predict(Output_arr.reshape(-1,4))
    ...
    error = error
    real = real_value
    model_purity = expit(y_prediction_purity)
    lst = Input
    lst.append(output[0])
    lst.append(Output[0])
    lst.append(error[0])
...
final_df.to_csv('result_constrain_low.csv')
